...
// TrySetCanonicalClient returns the canonical client for the rollapp. Empty if none set.
// Passed chain must be Dymension hub.
func TrySetCanonicalClient(ctx context.Context, c *Chain, clientID string) (string, error) {
	hub, ok := c.ChainProvider.(provider.DymensionHubProvider)
	if !ok {
		return "", errors.New("not hub query provider")
	}
	return hub.TrySetCanonicalClient(ctx, rollappID)
}
