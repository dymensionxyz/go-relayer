...
func (m *pathEndMessages) Nums() string {
	return fmt.Sprintf("connectionMessages: %d, channelMessages: %d, packetMessages: %d, clientICQMessages: %d", len(m.connectionMessages), len(m.channelMessages), len(m.packetMessages), len(m.clientICQMessages))
}
