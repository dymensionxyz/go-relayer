...
func ModifyGenesisStride() func(ibc.ChainConfig, []byte) ([]byte, error) {
	return func(cfg ibc.ChainConfig, genbz []byte) ([]byte, error) {
		...
		if err := json.Unmarshal(genbz, &g); err != nil {
			return nil, fmt.Errorf("to unmarshal genesis file: %w", err)
		}
		...
		if err := dyno.Set(g, VotingPeriod, "app_state", "gov", "voting_params", "voting_period"); err != nil {
			return nil, fmt.Errorf("to set voting period in genesis json: %w", err)
		}
		...
		if err != nil {
			return nil, fmt.Errorf("to marshal genesis bytes to json: %w", err)
		}
		return out, nil
	}
}
